...
function Translator:buildInput(tokens, srcDomain, tgtDomain)
  ...
  for b = 1, #src do
    if #src[b].words == 0 then
      ...
      if self.dicts.src.domains then
        table.insert(srcData.domains, self.dicts.src.domains:lookup(src[b].srcDomain))
      end
      if self.dicts.tgt.domains then
        table.insert(tgtData.domains, self.dicts.tgt.domains:lookup(src[b].tgtDomain))
      end
