...
function Translator:buildData(srcBatch, srcFeaturesBatch, srcDomainsBatch,
  ...
  for b = 1, #srcBatch do
    if #srcBatch[b] == 0 then
      ...
      if self.dicts.src.domains then
...
function Translator:translateBatch(batch)
  ...
  while remainingSents > 0 and i < self.opt.max_sent_length do
      ...
      if not beam[b].done then
        ...
        if self.dicts.tgt.domains then
          if inputDomain == nil then
            inputDomain = torch.IntTensor(self.opt.beam_size, remainingSents)
          end
          for k = 1, self.opt.beam_size do
    ...
    if inputDomain then
      inputDomain = inputDomain:view(self.opt.beam_size * remainingSents)
    end
...
function Translator:translate(srcBatch, srcFeaturesBatch, srcDomainsBatch,
                              goldBatch, goldFeaturesBatch, tgtDomainsBatch)
  local data, ignored = self:buildData(srcBatch, srcFeaturesBatch, srcDomainsBatch,
                                       goldBatch, goldFeaturesBatch, tgtDomainsBatch)
