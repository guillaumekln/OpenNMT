...
function DecoderAdvancer:initBeam()
  ...
  local domain
  if self.dicts and self.dicts.tgt.domains then
    domain = torch.IntTensor(self.batch.size)
    for b = 1, self.batch.size do
      domain[b] = self.batch.targetDomainInput[b]
    end
  end
  ...
  local state = { self.decStates, nil, self.context, nil, features, domain, sourceSizes, 1 }
...
function DecoderAdvancer:update(beam)
  ...
  local decStates, decOut, context, _, features, domain, sourceSizes, t
  ...
  local nextState = {decStates, decOut, context, softmaxOut, nil, domain, sourceSizes, t}
